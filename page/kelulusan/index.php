...
<div class="card">
  <div class="card-header">
    <h3 class="card-title">Proses Kelulusan Siswa</h3>
  </div>
  <!-- /.card-header -->
  <div class="card-body">
      ...
      <tbody>
      	...
      	?>
						...
						<div class="row">
									...
									<div class="col-md-4 mt-4">
										<div class="form-group">
											<button type="submit" name="naikan" class="btn btn-primary">Proses Kelulusan Siswa</button>
										</div>
									</div>
							</div>
					</form>
					...
					if(isset($_POST['naikan'])) {
						...
						for($i = 0; $i < $jml_pilih; $i++) {
							$naikan = $conn->query("UPDATE tb_siswa SET status_siswa = 'Lulus' WHERE id_siswa = '$siswa[$i]'") or die(mysqli_error($conn));
							if($naikan) {
								echo "<script>alert('Siswa berhasil di luluskan.');window.location='?p=kelulusan';</script>";
							} else {
								echo "<script>alert('Siswa gagal di luluskan.');window.location='?p=kelulusan';</script>";
							}
						}
					}
					?>
